...
Orc.Metadata is a library that provides classes to store metadata about objects. This metadata isn't necessarily stored inside the object itself, allowing the developer to keep track of additional information besides the information stored in the object.

The library is part of the [WildGums Open Source](http://opensource.wildgums.com) ecosystem.
...
3. **Submit a Pull Request** — Target the `develop` branch; changes must be reviewed by a human before merging
...
```
src/
  Orc.Metadata/            # Core library
  Orc.Metadata.Tests/      # Unit tests
```
...
| `src/Orc.Metadata/` | Yes | Core library source code |
| `src/Orc.Metadata.Tests/` | Yes | Test project |
...
| Changing or removing default parameters in public APIs | ABI breaking |
...
| Documentation portal | [opensource.wildgums.com](https://opensource.wildgums.com) |
